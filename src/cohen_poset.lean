...
noncomputable def size_of_domain : cohen_poset → ℕ :=
...
lemma empty_of_size_of_domain_0 {p : cohen_poset} (h : size_of_domain p = 0) : p.val.dom = ∅ :=
begin
  have : set.finite.to_finset p.property = ∅, from finset.card_eq_zero.mp h,
  exact finset.empty_of_empty this
end

lemma nonempty_of_size_of_domain_ne_zero {p : cohen_poset} (h : size_of_domain p ≠ 0) : nonempty p.val.dom :=
begin
...
def is_defined_at_covers {n} {h : 0 < n} : size_of_domain ⁻¹' {n} ⊆ ⋃ m, is_defined_at m :=
begin
...
instance size_of_domain_0_subsingleton : subsingleton $ size_of_domain ⁻¹' {0} :=
begin
  refine ⟨λ a b, _⟩, rcases a with ⟨⟨p_a, H_f_a⟩, H_a⟩, rcases b with ⟨⟨p_b, H_f_b⟩, H_b⟩, congr,
  have := (pfun.graph_empty_iff_dom_empty _).mpr (@empty_of_size_of_domain_0 _
...
lemma size_of_domain_0_inter_subsingleton : ∀ (a : set cohen_poset), subsingleton ↥((size_of_domain ⁻¹' {0}) ∩ a) :=
...
lemma one_point_restriction_finset_rewrite {p : cohen_poset} {x} {h : x ∈ p.val.dom} : set.finite.to_finset (one_point_restriction p x h).property = set.finite.to_finset (by {apply finite_of_inter_not_x, exact p.property} : set.finite $ p.val.dom ∩ not_x x) := by simp[one_point_restriction_domain]

lemma one_point_restriction_finset_rewrite_property {p : cohen_poset} {x} {h : x ∈ p.val.dom} : ∀ y, y ∈ set.finite.to_finset ((one_point_restriction p x h).property-- by {apply finite_of_inter_not_x, exact p.property} : set.finite $ p.val.dom ∩ not_x x
) ↔ y ≠ x ∧ y ∈ p.val.dom :=
begin
...
lemma one_point_restriction_erase {p : cohen_poset} {x} {h : x ∈ p.val.dom} : set.finite.to_finset (one_point_restriction p x h).property = finset.erase (set.finite.to_finset p.property) x :=
begin
