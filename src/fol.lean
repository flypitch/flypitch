...
lemma subst_fin_realize_eq {S : Type u} {n} {v₁ : dvector S n} {v₂ : ℕ → S}
  (hv : ∀k, v₁.nth2 k = v₂ k.to_nat) (x : S) (k : dfin (n+1)) :
    (x::v₁).nth2 k = v₂[x // 0] k.to_nat :=
begin
  cases k; simp [hv],
  rw [subst_realize_gt _ _ (nat.succ_pos _)], refl
end
...
@[simp] def is_bounded_preterm (n : ℕ) : ∀ {l}, preterm L l → Prop
| _ (var k) := k < n
| _ (func f) := true
| _ (app t s) := is_bounded_preterm t ∧ is_bounded_preterm s

variable (L)
inductive bounded_preterm (n : ℕ) : ℕ → Type u
| bd_var {} : ∀ (k : dfin n), bounded_preterm 0
...
namespace is_bounded_preterm

theorem mono {m n} (mn : m ≤ n) : ∀ {l} {t : preterm L l},
  is_bounded_preterm m t → is_bounded_preterm n t
| _ (var k) h := lt_of_lt_of_le h mn
| _ (func f) h := trivial
| _ (app t s) h := h.imp mono mono

lemma lift_irrel {n : ℕ} : ∀{l} {t : preterm L l} (ht : is_bounded_preterm n t)
  (n') {m : ℕ} (h : n ≤ m), t ↑' n' # m = t
| _ &k           ht n' m h :=
  have ¬ m ≤ k, by simpa using mono h ht,
  by simp [this]
| _ (func f)  ht n' m h := by refl
| _ (app t s) ht n' m h := by simp [lift_irrel ht.1 n' h, lift_irrel ht.2 n' h]

lemma subst_irrel {n : ℕ} : ∀{l} {t : preterm L l} (ht : is_bounded_preterm n t) {n'} (s : term L)
  (h : n ≤ n'), t[s // n'] = t
| _ &k          ht n' s h := by simp [lt_of_lt_of_le ht h]
| _ (func f)    ht n' s h := rfl
| _ (app t₁ t₂) ht n' s h := by simp [subst_irrel ht.1 s h, subst_irrel ht.2 s h]

theorem lift_at {n} : ∀{l} {t : preterm L l} (ht : is_bounded_preterm n t)
  {n' m : ℕ}, is_bounded_preterm (n + n') (t ↑' n' # m)
| _ &k          ht n' m := show _<_, by split_ifs; [
  exact add_lt_add_right ht _,
  exact lt_of_lt_of_le ht (nat.le_add_right _ _)]
| _ (func f)    ht n' m := ht
| _ (app t₁ t₂) ht n' m := ⟨ht.1.lift_at, ht.2.lift_at⟩

theorem lift {n l} {t : preterm L l}
  (ht : is_bounded_preterm n t) {n'} : is_bounded_preterm (n + n') (t ↑ n') :=
ht.lift_at

/-- this is t[s//n] for bounded formulae -/
theorem subst {m n} : ∀{l} {t : preterm L l} (ht : is_bounded_preterm (n+m+1) t) {s : term L}, is_bounded_preterm m s → is_bounded_preterm (n+m) (t[s // n])
| _ &k          ht s hs := begin
    unfold subst_term,
    rcases lt_trichotomy n k with h|rfl|h,
    { simp [h],
      have : 0 < k := lt_of_le_of_lt (nat.zero_le _) h,
      rw[add_comm], exact (nat.pred_lt_iff_lt_succ this).2 ht
      },
    { simp, exact hs.lift },
    { simp [h], exact lt_of_lt_of_le h (nat.le_add_left _ _) }
  end
| _ (func f)    ht s hs := ht
| _ (app t₁ t₂) ht s hs := ⟨ht.1.subst hs, ht.2.subst hs⟩

end is_bounded_preterm
...
@[simp]def mk {n : ℕ} : ∀ {l} (t : preterm L l),
  is_bounded_preterm n t → bounded_preterm L n l
| _ (var k) h := bd_var (dfin.of_nat_lt k h)
| _ (func f) h := bd_func f
| _ (app t s) h := bd_app (mk t h.1) (mk s h.2)

@[simp] theorem fst_mk {n : ℕ} : ∀ {l} {t : preterm L l}
  (h : is_bounded_preterm n t), (mk t h).fst = t
| _ (var k) h := by simp
| _ (func f) h := rfl
| _ (app t s) h := by simp*

@[simp] theorem fst_bounded {n : ℕ} : ∀ {l} (t : bounded_preterm L n l),
  is_bounded_preterm n t.fst
| _ &k           := k.to_nat_lt
| _ (bd_func f)  := trivial
| _ (bd_app t s) := ⟨fst_bounded t, fst_bounded s⟩

local attribute [extensionality] fin.eq_of_veq
@[extensionality] protected def eq {n} : ∀{l} {t₁ t₂ : bounded_preterm L n l} (h : t₁.fst = t₂.fst),
  t₁ = t₂
| _ &k &k'                        h := by injection h with h'; congr1; exact dfin.to_nat_inj h'
...
@[simp] lemma cast_irrel {n m } {h h' : n ≤ m} {l} (t : bounded_preterm L n l) : t.cast h = t.cast h' := rfl
...
def realize_is_bounded_term {S : Structure L} {n} (v : dvector S n)
  {l} (t : preterm L l) (ht : is_bounded_preterm n t) : dvector S l → S.carrier :=
realize_bounded_term v (bounded_preterm.mk t ht)

lemma realize_bounded_term_eq {S : Structure L} {n} {v₁ : dvector S n} {v₂ : ℕ → S}
  (hv : ∀k, v₁.nth2 k = v₂ k.to_nat) : ∀{l} (t : bounded_preterm L n l)
...
lemma realize_is_bounded_term_eq {S : Structure L} {n} {v₁ : dvector S n} {v₂ : ℕ → S}
  (hv : ∀k, v₁.nth2 k = v₂ k.to_nat) {l} {t : preterm L l}
  (ht : is_bounded_preterm n t) (xs : dvector S l) :
  realize_is_bounded_term v₁ t ht xs = realize_term v₂ t xs :=
by rw [realize_is_bounded_term, realize_bounded_term_eq hv, bounded_preterm.fst_mk]

lemma realize_bounded_term_eq' {S : Structure L} {n} {v : dvector S n}
  {l} (t : bounded_preterm L n l) (xs : dvector S l) :
  realize_bounded_term v t xs = realize_is_bounded_term v t.fst t.fst_bounded xs :=
by rw [realize_is_bounded_term, bounded_preterm.mk_fst]

-- lemma realize_bounded_term_irrel' {S : Structure L} {n n'} {v₁ : dvector S n} {v₂ : dvector S n'}
--   (h : ∀m (hn : m < n) (hn' : m < n'), v₁.nth2 (dfin.of_nat_lt m hn) = v₂.nth2 (dfin.of_nat_lt m hn'))
...
lemma realize_bounded_term_irrel_le {S : Structure L} {n n'} (hn : n ≤ n') {v₁ : dvector S n} {v₂ : dvector S n'}
  (h : ∀m, v₁.nth2 m = v₂.nth2 (m.cast_le hn)) :
  ∀ {l} {t : preterm L l} (ht : is_bounded_preterm n t) (xs : dvector S l),
  realize_is_bounded_term v₁ t ht xs = realize_is_bounded_term v₂ t (ht.mono hn) xs
| _ &k          ht xs := by simp [realize_is_bounded_term, realize_bounded_term];
  rw h; congr; ext; simp
| _ (func f)    ht xs := rfl
| _ (app t₁ t₂) ht xs := begin
    have IH1 := realize_bounded_term_irrel_le ht.1,
    have IH2 := realize_bounded_term_irrel_le ht.2,
    unfold realize_is_bounded_term at IH1 IH2 ⊢,
    simp [realize_bounded_term, IH1, IH2]
  end
...
begin
  have : t = t'.cast (nat.zero_le n), {ext, simp [ht]}, subst t,
  rw [realize_bounded_term_eq', ← realize_bounded_term_irrel_le (nat.zero_le _) (λ m, m.elim0)],
  { simp [realize_bounded_term_eq', realize_closed_term],
    cases xs, refl },
  { simp },
  { exact xs }
end
...
realize_bounded_term v (t.cast_eq h) xs = realize_bounded_term (v.cast h.symm) t xs := by {subst h, induction t, sorry, refl, simp*}
...
bounded_preterm.mk (t.fst ↑' n' # m) t.fst_bounded.lift_at
...
@[simp] lemma lift_bounded_term_fst {n l} (t : bounded_preterm L n l) (n' m : ℕ) :
  (t ↑' n' # m).fst = t.fst ↑' n' # m :=
bounded_preterm.fst_mk _

@[simp] def lift_closed_term_at {l} (t : closed_preterm L l) (n' m : ℕ) :
  bounded_preterm L n' l :=
bounded_preterm.mk (t.fst ↑' n' # m) (by simpa using t.fst_bounded.lift_at)
...
bounded_preterm.mk (t.fst[s.fst//n]) (t.fst_bounded.subst s.fst_bounded)
...
def subst0_bounded_term {n l} (t : bounded_preterm L (n+1) l) (s : bounded_term L n) :
  ...
  bounded_preterm.mk (t.fst[s.fst//0]) $
                     by {have : n = 0 + n, by rw[zero_add],
                        conv {congr, rw[this]},
                        refine @is_bounded_preterm.subst _ n 0 _ _ _ _ _, rw[zero_add],
...
def substmax_var_lt {n} (k : dfin (n+1)) (s : closed_term L) (h : k.to_nat < n) :
  substmax_bounded_term &k s = &(dfin.of_nat_lt k.to_nat (by {exact h})) :=
by ext; simp [substmax_bounded_term, h]

def substmax_var_eq {n} (k : dfin (n+1)) (s : closed_term L) (h : k.to_nat = n) :
...
-- begin
--   revert t, refine bounded_term.rec (λ k, k.elim0) _,
...
def is_bounded_preformula : ∀ (n : ℕ) {l}, preformula L l → Prop
| n _ falsum := true
| n _ (equal t₁ t₂) := is_bounded_preterm n t₁ ∧ is_bounded_preterm n t₂
| n _ (rel R) := true
| n _ (apprel f t) := is_bounded_preformula n f ∧ is_bounded_preterm n t
| n _ (imp f₁ f₂) := is_bounded_preformula n f₁ ∧ is_bounded_preformula n f₂
| n _ (all f) := is_bounded_preformula (n+1) f
...
namespace is_bounded_preformula

theorem mono : ∀ {m n}, m ≤ n → ∀ {l} {t : preformula L l},
  is_bounded_preformula m t → is_bounded_preformula n t
| m n mn _ falsum       h := h
| m n mn _ (t₁ ≃ t₂)    h := ⟨h.1.mono mn, h.2.mono mn⟩
| m n mn _ (rel R)      h := h
| m n mn _ (apprel f t) h :=  ⟨h.1.mono mn, h.2.mono mn⟩
| m n mn _ (f₁ ⟹ f₂)    h :=  ⟨h.1.mono mn, h.2.mono mn⟩
| m n mn _ (∀' f)       h :=  mono (succ_le_succ mn) h

lemma lift_irrel : ∀ {n : ℕ} {l} {t : preformula L l} (ht : is_bounded_preformula n t)
  (n') {m : ℕ} (h : n ≤ m), t ↑' n' # m = t
| n _ falsum       ht n' m h := rfl
| n _ (t₁ ≃ t₂)    ht n' m h := by simp [ht.1.lift_irrel _ h, ht.2.lift_irrel _ h]
| n _ (rel R)      ht n' m h := rfl
| n _ (apprel f t) ht n' m h := by simp [ht.1.lift_irrel _ h, ht.2.lift_irrel _ h]
| n _ (f₁ ⟹ f₂)    ht n' m h := by simp [ht.1.lift_irrel _ h, ht.2.lift_irrel _ h]
| n _ (∀' f)       ht n' m h := by simp; exact lift_irrel (by exact ht) _ (succ_le_succ h)

lemma subst_irrel : ∀ {n : ℕ} {l} {t : preformula L l} (ht : is_bounded_preformula n t) {n'} (s : term L)
  (h : n ≤ n'), t[s // n'] = t
| n _ falsum       ht n' s h := rfl
| n _ (t₁ ≃ t₂)    ht n' s h := by simp [ht.1.subst_irrel _ h, ht.2.subst_irrel _ h]
| n _ (rel R)      ht n' s h := rfl
| n _ (apprel f t) ht n' s h := by simp [ht.1.subst_irrel _ h, ht.2.subst_irrel _ h]
...
theorem subst : ∀ {n' n l} {f : preformula L l} (hf : is_bounded_preformula (n+n'+1) f)
  {s : term L} (hs : is_bounded_preterm n' s), is_bounded_preformula (n+n') (f[s//n])
:=
begin
  intros, induction f generalizing n,
  {trivial},
  {split; apply is_bounded_preterm.subst, tidy},
  {trivial},
  {split; try{apply is_bounded_preterm.subst}; tidy},
  {tidy},
  {dsimp[is_bounded_preformula],
  rw[(by simp : n + n' + 1 = n + 1 + n')], apply @f_ih (n+1),
  rw[(by simp : n + 1 + n' + 1 = n + n' + 1 + 1)], exact hf},
end
-- | n' n l falsum hf s hs := trivial
-- | n' n l (t₁ ≃ t₂) hf s hs := by {split; apply is_bounded_preterm.subst, tidy}
-- | n' n l (rel R) _ _ _ := trivial
-- | n' n l (apprel f t) hf s hs := by {split; try{apply is_bounded_preterm.subst}; tidy}
-- | n' n l (f₁ ⟹ f₂) hf s hs := by tidy
-- | n' n l (∀' f) hf s hs := by {dsimp[is_bounded_preformula], have := @subst n' (n+1) _ f _ s _,
--                                rw[(by simp : n + n' + 1 = n + 1 + n')], apply this,
--                                rw[(by simp : n + 1 + n' + 1 = n + n' + 1 + 1)], exact hf,
--                                exact hs
--                                }
                              

--dsimp[is_bounded_preformula], have : n + n' + 1 = n + 1 + n',
                               -- by simp, rw[this], apply @subst n' (n+1) 0 f _ s _,
                               -- rw[<-this], repeat{assumption}
-- | n n' l f bd_falsum       s rfl := ⊥
...
@[simp] lemma fst_not {n} {f : bounded_formula L n} :
  ∼(bounded_preformula.fst f) = bounded_preformula.fst (∼f) := rfl

@[simp] def mk : ∀ {n : ℕ} {l} (t : preformula L l),
  is_bounded_preformula n t → bounded_preformula L n l
| n _ falsum       h := bd_falsum
| n _ (t₁ ≃ t₂)    h := bounded_preterm.mk t₁ h.1 ≃ bounded_preterm.mk t₂ h.2
| n _ (rel R)      h := bd_rel R
| n _ (apprel f t) h := bd_apprel (mk f h.1) (bounded_preterm.mk t h.2)
| n _ (f₁ ⟹ f₂)    h := mk f₁ h.1 ⟹ mk f₂ h.2
| n _ (∀' f)       h := ∀' mk f h

@[simp] theorem fst_mk : ∀ {n : ℕ} {l} {t : preformula L l}
  (h : is_bounded_preformula n t), (mk t h).fst = t
| n _ falsum       h := rfl
| n _ (t₁ ≃ t₂)    h := by simp [h.1, h.2]
| n _ (rel R)      h := rfl
| n _ (apprel f t) h := by simp [h.1, h.2, fst_mk]
| n _ (f₁ ⟹ f₂)    h := by simp [h.1, h.2, fst_mk]
| n _ (∀' f)       h := by simp [@fst_mk _ _ f h]

@[simp] theorem fst_bounded : ∀ {n : ℕ} {l} (t : bounded_preformula L n l),
  is_bounded_preformula n t.fst
| _ _ bd_falsum       := trivial
| _ _ (t₁ ≃ t₂)       := ⟨t₁.fst_bounded, t₂.fst_bounded⟩
| _ _ (bd_rel R)      := trivial
| _ _ (bd_apprel f t) := ⟨f.fst_bounded, t.fst_bounded⟩
| _ _ (f₁ ⟹ f₂)      := ⟨f₁.fst_bounded, f₂.fst_bounded⟩
| _ _ (∀' f)          := f.fst_bounded
...
def realize_is_bounded_formula {S : Structure L} {n} (v : dvector S n)
  {l} (t : preformula L l) (ht : is_bounded_preformula n t) : dvector S l → Prop :=
realize_bounded_formula v (bounded_preformula.mk t ht)

lemma realize_bounded_formula_iff {S : Structure L} : ∀{n} {v₁ : dvector S n} {v₂ : ℕ → S}
  (hv : ∀k, v₁.nth2 k = v₂ k.to_nat) {l} (t : bounded_preformula L n l)
  (xs : dvector S l), realize_bounded_formula v₁ t xs ↔ realize_formula v₂ t.fst xs
| n v₁ v₂ hv _ bd_falsum       xs := iff.rfl
| n v₁ v₂ hv _ (t₁ ≃ t₂)       xs := by apply eq.congr; apply realize_bounded_term_eq hv
| n v₁ v₂ hv _ (bd_rel R)      xs := iff.rfl
| n v₁ v₂ hv _ (bd_apprel f t) xs :=
  by simp [realize_bounded_term_eq hv, realize_bounded_formula_iff hv]
| n v₁ v₂ hv _ (f₁ ⟹ f₂)      xs :=
  by simp [realize_bounded_formula_iff hv]
| n v₁ v₂ hv _ (∀' f)          xs :=
  forall_congr $ λ x, begin
    apply realize_bounded_formula_iff,
    rintro (_|k); simp, apply hv
  end

lemma realize_is_bounded_formula_iff {S : Structure L} {n} {v₁ : dvector S n} {v₂ : ℕ → S}
  (hv : ∀k, v₁.nth2 k = v₂ k.to_nat) {l} {t : preformula L l}
  (ht : is_bounded_preformula n t) (xs : dvector S l) :
  realize_is_bounded_formula v₁ t ht xs ↔ realize_formula v₂ t xs :=
by rw [realize_is_bounded_formula, realize_bounded_formula_iff hv, bounded_preformula.fst_mk]

lemma realize_bounded_formula_iff' {S : Structure L} {n} {v : dvector S n}
  {l} (t : bounded_preformula L n l) (xs : dvector S l) :
...
@[simp] def subst_bounded_formula {n n' l} (f : bounded_preformula L (n + n' + 1) l)
  (s : bounded_term L n') : bounded_preformula L (n + n') l
  := bounded_preformula.mk (by {exact f.fst[s.fst//n]}) (f.fst_bounded.subst s.fst_bounded)

-- bounded_preterm.mk (t.fst[s.fst//n]) (t.fst_bounded.subst s.fst_bounded)
-- | _ _ _ _ bd_falsum       s rfl := bd_falsum
-- | _ _ _ l (t₁ ≃ t₂)       s rfl := (subst_bounded_term t₁ s) ≃ (subst_bounded_term t₂ s)
-- | _ _ _ _ (bd_rel R)      s rfl := bd_rel R
-- | n' n n'' l (bd_apprel f t) s rfl := bd_apprel (@subst_bounded_formula _ _ _ _ f s rfl) (subst_bounded_term t s)
-- | n' n n'' l (f₁ ⟹ f₂)      s rfl := (@subst_bounded_formula _ _ _ _ f₁ s rfl)  ⟹ (@subst_bounded_formula _ _ _ _ f₂ s rfl)
-- | n' n n'' l (∀' f)          s rfl  := ∀' (@subst_bounded_formula (n') (n+1) _ 0 f s $ by {simp})


-- (subst_bounded_formula f s $ by simp [succ_add]).cast_eq (succ_add _ _)

notation f`[`:max s ` !! `:90 n `]`:0 := @_root_.fol.subst_bounded_formula _ n _ _ f s

-- notation f `[`:95 s ` // `:95 n ` // `:95 h `]`:0 := @fol.subst_bounded_formula _ _ n _ _ f s h

@[simp] def subst_bounded_formula_fst : ∀{n n' l} (f : bounded_preformula L (n + n' + 1) l)
  (s : bounded_term L n'),
  (subst_bounded_formula f s).fst = f.fst[s.fst//n]
| _ _ _ bd_falsum       s  := by refl
| _ _ _ (t₁ ≃ t₂)       s  := by simp
| _ _ _ (bd_rel R)      s  := by refl
| _ _ _ (bd_apprel f t) s  := by simp*
| _ _ _ (f₁ ⟹ f₂)      s  := by simp*
| _ _ _ (∀' f)          s  := by simp*
...
lemma realize_bounded_formula_irrel_le {S : Structure L} {n n'} (hn : n ≤ n') {v₁ : dvector S n} {v₂ : dvector S n'}
  (h : ∀m, v₁.nth2 m = v₂.nth2 (m.cast_le hn))
  {l} (f : preformula L l) (hf : is_bounded_preformula n f) (xs : dvector S l) :
  realize_is_bounded_formula v₁ f hf xs ↔ realize_is_bounded_formula v₂ f (hf.mono hn) xs :=
begin
  have tm := @realize_bounded_term_irrel_le _ S,
  delta realize_is_bounded_term at tm,
  unfold realize_is_bounded_formula,
  induction f generalizing n n',
  { refl },
  { simp [tm hn h hf.1, tm hn h hf.2] },
  { refl },
  { simp [f_ih _ hn h hf.1, tm hn h hf.2] },
  { simp [f_ih_f₁ _ hn h hf.1, f_ih_f₂ _ hn h hf.2] },
  { apply forall_congr, intro x, apply f_ih _ (succ_le_succ hn) _ hf,
    rintro (_|k), {refl}, {apply h} }
end
...
begin
  have : f = f'.cast (nat.zero_le n), {ext, simp [hf]}, subst f,
  rw [realize_bounded_formula_iff', ← realize_bounded_formula_irrel_le (nat.zero_le _) (λ m, m.elim0)],
  { simp [realize_bounded_formula_iff', realize_sentence],
    cases xs, refl },
  { simp },
  { exact xs }
end
...
@[simp] def substmax_bounded_formula {n l} (f : bounded_preformula L (n+1) l) (s : closed_term L) :
  ...
  @subst_bounded_formula L n 0 l f s
...
def subst0_bounded_formula {n l} (f : bounded_preformula L (n+1) l) (s : bounded_term L n) :
  bounded_preformula L n l :=
    bounded_preformula.mk (f.fst[s.fst//0]) $
                        by {have : n = 0 + n, by rw[zero_add], conv {congr, rw[this]},
                          refine @is_bounded_preformula.subst L n 0 _ _ _ _ _, rw[zero_add],
                         exacts [f.fst_bounded, s.fst_bounded]}

-- @subst_bounded_formula L n 0 _ f s
...
realize_bounded_formula_iff (λk, k.elim0) f _
...
def subst_var_bounded_term {n m : ℕ} : ∀ {l : ℕ}, bounded_preterm L n l → dvector (dfin m) n → bounded_preterm L m l
  | _ (&k) v := &(dvector.nth v k.to_nat k.to_nat_lt)
...
begin
  induction t,
  { by_cases h : t.to_nat < n,
    { rw [substmax_var_lt t s h], dsimp,
      sorry},
    { have h' := le_antisymm (le_of_lt_succ t.to_nat_lt) (le_of_not_gt h), simp [h'],
      rw [substmax_var_eq t s h'],
      sorry -- apply realize_bounded_term_irrel, simp
      }},
  { refl },
