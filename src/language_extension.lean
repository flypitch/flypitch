...
def language_morphism (L L' : Language) : Type := Π n : ℕ, (L.relations n → L'.relations n) × (L.functions n → L'.functions n)
...
def Language_over : Type 2 := Σ L' : Language, language_morphism L L'
