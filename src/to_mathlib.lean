...
/--
  `change' n g` attempts to change the current goal to g,
...
/--
  ac_change' uses change' and attempts to close any goals generated by `congr'` using `ac_refl`
-/
meta def ac_change' (k : parse (with_desc "n" small_nat)?) (r : parse texpr) : tactic unit :=
match k with
| none := `[change' 1 %%r; try {ac_refl}]
| o := change' o r; try ac_refl
end
