...
/--
  `convert_to n g` attempts to change the current goal to g,
  using `congr' n` to resolve discrepancies
-/
meta def convert_to (k : parse (with_desc "n" small_nat)?) (r : parse texpr) : tactic unit :=
do
  tgt <- target,
  h <- to_expr ``(_ : %%tgt = %%r),
...
/--
  ac_change' uses convert_to and attempts to close any goals generated by `congr'` using `ac_refl`
-/
meta def ac_change' (k : parse (with_desc "n" small_nat)?) (r : parse texpr) : tactic unit :=
match k with
| none := `[convert_to 1 %%r; try {ac_refl}]
| o := convert_to o r; try ac_refl
end
