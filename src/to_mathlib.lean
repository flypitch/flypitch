...
/-- `ac_change g' changes the current goal `tgt` to `g` by
     ...
     attempt to close discrepancies with `ac_refl`. -/
...
/--
  `change_congr n g` attempts to change the current goal to g,
  using `congr' n` to resolve discrepancies
-/
meta def change_congr (k : parse (with_desc "n" small_nat)?) (r : parse texpr) : tactic unit :=
do
  tgt <- target,
  h <- to_expr ``(_ : %%tgt = %%r),
...
/--
  ac_change' uses change_congr and attempts to close any goals generated by `congr'` using `ac_refl`
-/
meta def ac_change' (k : parse (with_desc "n" small_nat)?) (r : parse texpr) : tactic unit :=
match k with
| none := `[change_congr 1 %%r; try {ac_refl}]
| o := change_congr o r; try ac_refl
end

-- example {α : Type*} [lattice.boolean_algebra α] {a₁ a₂ a₃ a₄ : α} (h : a₁ = ⊤) :
--   (a₁ ⊔ a₂) ⊔ (a₃ ⊔ a₄) = (a₂ ⊔ a₁) ⊔ (a₄ ⊔ a₃)
-- :=
-- begin
--   ac_change' (a₁ ⊔ a₂) ⊔ (a₃ ⊔ a₄) = (a₁ ⊔ a₂) ⊔ (a₃ ⊔ a₄)
-- end
...
-- :=
-- begin
--   ac_change (a₁ ⊔ a₂) ⊔ (a₃ ⊔ a₄) = (a₁ ⊔ a₂) ⊔ (a₃ ⊔ a₄), refl
-- end
...
[ reflexivity >> pure "refl",
 propositional_goal >> assumption >> pure "assumption",
  ...
  `[exact bv_refl]        >> pure "exact bv_eq_refl _",
