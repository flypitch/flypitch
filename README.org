#+TITLE: Flypitch
*** The result
In this repository we have given a formal proof of the [[https://en.wikipedia.org/wiki/Continuum_hypothesis#Independence_from_ZFC][independence of the continuum hypothesis]].
...
- ZFC' is the theory of Zermelo-Fraenkel set theory, defined in [[src/zfc'.lean]].
  - We formulate it in a language with 5 constant/function symbols (in addition the usual elementhood relation): for the empty set, pairs, omega, the power set and union.
  - For each constant/function symbol ZFC' contains an axiom giving the defining property of that symbol. Addionally, it contains the axiom of extensionality, regularity, Zorn's lemma and the axiom schema of collection.
  - The 5 constant/function symbols we added are definable in ZFC, so ZFC' is conservative over ZFC. But these extra symbols make the formulation of Zorn's lemma and the continuum hypothesis easier. We plan to also formalize the reduction from ZFC' to ZFC.
- ~CH_f~ is the continuum hypothesis as a first-order logic sentence. The sentence we use is
  ~∀x, x is an ordinal ⟹ x ≤ ω ∨ P(ω) ≤ x~ where ~a ≤ b~ means that there is a surjection from a subset of ~b~ to ~a~.
